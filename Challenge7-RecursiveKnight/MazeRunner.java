...
//		 TODO: Implement moveOne
	...
	public boolean caught(MazeRunner other) {
		...
		if((x == other.x)==false || (y==other.y)==false){
	...
	static int findShortestString(String[] paths, int lo, int hi) {
		...
		if(safeStringLength(paths[findShortestString(paths, lo+1,hi)])<safeStringLength(paths[lo])){
	...
	public static String shortestPath(int x, int y, int tX, int tY,
		...
		if (blocked[x][y] == true || blocked[tX][tY] == true){
			return null;
		}
		if (x == tX && y == tY){
			return "";
		...
		blocked[x][y] = true;
		String[] result = {shortestPath(x,y+1,tX,tY,blocked),shortestPath(x,y-1,tX,tY,blocked),shortestPath(x+1,y,tX,tY,blocked),shortestPath(x-1,y,tX,tY,blocked)};
		String[] names = {"N", "S", "E", "W"};
		blocked[x][y] = false;
		if((result[findShortestString(result,0,result.length-1)] == null)==false && findShortestString(result,0,result.length-1)>=0){
			return names[findShortestString(result,0,result.length-1)] + result[findShortestString(result,0,result.length-1)];
			}
		else{
			return null;
		}
	...
	public void chase(boolean maze[][], int targetX, int targetY) {
		...
		// TODO: Implement chase
        // Use shortestPath, string.charAt,  moveOne
        String shortestp = shortestPath(x, y, targetX, targetY, maze);
        if((shortestp == "")==false && ((x == targetX)==false || (y == targetY)==false)||shortestp.length()>0 ){
                moveOne(shortestp.charAt(0));
        }
        else{
        	;
        }
